...
_SAMPLEDICT = {
	'condTest' : {
		...
		#	'TiBkgd' : [146166],
			'Au' : [146016],
			'Cu' : [146008, 146009, 146011, 146012]
		}
	}
}
'''
...
def loadDATA(NPZlist):
	...
	for NPZnumber in NPZlist:
		ImPath = os.path.realpath("D:\\aa_2017-04_i06_si16373-1\\si16373-1\\" + str(NPZnumber) +'_PCOImage' + '\\'+ str(NPZnumber) + '_PCOImage.npz')
...
if __name__ == "__main__":
	...
	_SAMPLENAME, _SAMPLEDICT, = dictionaryV2.sampleC()
	...
	_ROOTPATH = os.path.realpath("D:\\aa_2017-04_i06_si16373-1\\si16373-1")
	anaName = '{}_{}'.format(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'), _SAMPLENAME)
	anaPath = os.path.realpath(_ROOTPATH + "\\{}".format(anaName))
	
	X1, X2, delta = 50, 670, 30
	#print('X1:', X1-20, X1+20, 'X1:', X2-20, X2+20)
	ndata = int(sqrt((_PCONFIG[0]-_PCONFIG[1])**2+(_PCONFIG[2]-_PCONFIG[3])**2))
	_PEAKPROFILE = numpy.column_stack((numpy.linspace(0,ndata-1,ndata),numpy.zeros(ndata)))
	_AuAllPROFILE = numpy.linspace(0,ndata-1,ndata)
	_CuAllPROFILE = numpy.linspace(0,ndata-1,ndata)
	headerAuAllPEAK = 'Pixel'
	headerCuAllPEAK = 'Pixel'
	...
	loadDATA(AuBkgd)
	GetProfile()
	Y1, Y2 = _PEAKPROFILE[:,1][X1-delta:X1+delta].mean(), _PEAKPROFILE[:,1][X2-delta:X2+delta].mean()
	i = 0
	
	for i in range(len(_PEAKPROFILE[:,1])):
		_PEAKPROFILE[:,1][i] = _PEAKPROFILE[:,1][i]-((Y2-Y1)/(X2-X1))*i
	_PEAKPROFILE[:,1] = _PEAKPROFILE[:,1]*(1000/_PEAKPROFILE[:,1][X1-delta:X1+delta].mean())
	...
	for Kcondition, Dzone in _SAMPLEDICT.items():
		for Kzone, Dpeak in Dzone.items():
			for Kpeak, VnpzList in Dpeak.items():
				...
				headerPEAK = 'Pixel\t{}-{}-{}-{}'.format(_SAMPLENAME, Kcondition, Kzone, Kpeak)
				
				fmtPEAK = ['%d', '%f'] 
				
				if _KPEAK == "Au" :
					headerAuAllPEAK += '\t{0}-{1}-{2}-{3}-substract'.format(_SAMPLENAME, Kcondition, Kzone, Kpeak)
					_AuAllPROFILE = numpy.column_stack((_AuAllPROFILE,_PEAKPROFILE[:,1] - _AuBkgdPROFILE[:,1]))
					_PEAKPROFILE = numpy.column_stack((_PEAKPROFILE, _AuBkgdPROFILE[:,1],_PEAKPROFILE[:,1] - _AuBkgdPROFILE[:,1]))
					
					fmtPEAK += ['%f', '%f']
					headerPEAK = headerPEAK + '\t{0}-{1}-{2}-{3}-bkgd\t{0}-{1}-{2}-{3}-substract'.format(_SAMPLENAME, Kcondition, Kzone, Kpeak)
				elif _KPEAK == "Cu" :
					headerCuAllPEAK += '\t{0}-{1}-{2}-{3}-substract'.format(_SAMPLENAME, Kcondition, Kzone, Kpeak)
					_CuAllPROFILE = numpy.column_stack((_CuAllPROFILE,_PEAKPROFILE[:,1] - _CuBkgdPROFILE[:,1]))
				...
				if _FLAGSAVE == True:
					_PROFILEDOTDAT = os.path.join(peakPath + '\\{}-{}-{}-{}.profile'.format(_SAMPLENAME, Kcondition, Kzone, Kpeak))
					numpy.savetxt(_PROFILEDOTDAT, _PEAKPROFILE, header = headerPEAK, delimiter = '\t', comments='', fmt=fmtPEAK)
	
	numpy.savetxt(os.path.realpath(_ROOTPATH + "\\{}".format(anaName) + "\\Au.profile"), _AuAllPROFILE, header = headerAuAllPEAK, delimiter = '\t', comments='')#, fmt=fmtPEAK)
	numpy.savetxt(os.path.realpath(_ROOTPATH + "\\{}".format(anaName) + "\\Cu.profile"), _CuAllPROFILE, header = headerCuAllPEAK, delimiter = '\t', comments='')#, fmt=fmtPEAK)